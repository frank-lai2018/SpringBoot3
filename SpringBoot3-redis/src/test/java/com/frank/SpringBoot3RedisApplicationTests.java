...
class SpringBoot3RedisApplicationTests {
	...
	@Autowired // key.value 都是字串
	StringRedisTemplate redisTemplate;

	/**
	 * string： 普通字串 ： redisTemplate.opsForValue()
	 */
	@Test
	void contextLoads() {
	...
	void testzset() {
		...
		redisTemplate.opsForZSet().add(setName, "frank", 90.00);
		redisTemplate.opsForZSet().add(setName, "張三", 99.00);
	...
	/**
	 * hash： map結構： redisTemplate.opsForHash()
	 */
	@Test
	void testhash() {
		String mapName = "amap";
		redisTemplate.opsForHash().put(mapName, "name", "張三");
